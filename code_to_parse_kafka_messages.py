...
def limit_characters(data_frame):
    if not data_frame.empty:
        for col in data_frame.columns:
            if data_frame[col].dtypes == 'object':
                data_frame[col] = data_frame[col].str[:3500]
    return data_frame
...
class Json_to_csv:
    ...
    def log_to_db(self, flag, error_description=None, error_section=None):
        config_properties = collect_property_file_contents('config.ini', 'QUERY')
    ...
    def insert_general_kafka(self, flag, csv_filename):
        if flag in self.deal and self.deal[flag] != {}:
            constant = collect_property_file_contents('constant.ini', flag.upper())
            ...
            data_parser = limit_characters(data_parser)
            self.insert_kafka_offset_columns(data_parser)
            data_parser.replace('"', '', regex=True, inplace=True)
    ...
    def insert_st_int_cq_deal_kafka(self):
        ...
        constant = collect_property_file_contents('constant.ini', 'DEAL')
    ...
    def insert_st_int_cq_single_quote_vw_kafka(self, quotes_list):
        if 'quotes' in self.deal:
            constant = collect_property_file_contents('constant.ini', 'QUOTES')
        ...
        for _, quotes in enumerate(self.deal['quotes']):
            if 'standardApprovalRouteInfo' in quotes and quotes['standardApprovalRouteInfo'] != {}:
                if 'BR' in quotes['standardApprovalRouteInfo'].keys():
                    ...
                    flag = ''
                constant = collect_property_file_contents('constant.ini',
                ...
                if 'PARTNER' in quotes['quoteContacts']:
                    ...
                    for part in partner:
                        ...
                        if not data_parser1.empty:
                            ...
                            data_parser1.replace('"', '', regex=True, inplace=True)
                            data_parser1.replace('\n\n|\n|\r\n', ' ', regex=True, inplace=True)
                            data_parser1 = limit_characters(data_parser1)
                ...
                if 'ENDCUSTOMER' in quotes['quoteContacts']:
                    ...
                    for end in end_customer:
                        ...
                        if not data_parser2.empty:
                            ...
                            data_parser2.replace('"', '', regex=True, inplace=True)
                            data_parser2.replace('\n\n|\n|\r\n', ' ', regex=True, inplace=True)
                            data_parser2 = limit_characters(data_parser2)
        ...
        if in_rec:
            if 'childLines' in child_line:
                for _, child in enumerate(child_line['childLines']):
                    ...
                    self.recursive_child_concessions(child, constant_keys, quotes)
        else:
            self.recursive_child_concessions(child_line, constant_keys, quotes, in_rec=False)
